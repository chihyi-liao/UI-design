...
class CandlestickWidget(QWidget):
    def __init__(
        ...
        self.setObjectName("FinanceFrame")
        self.data = data
        self.title = title
        ...
        if self.data is None or self.data.empty:
            self.placeholder_label = QLabel("暫無資料", self)
            self.widget_layout.addWidget(self.placeholder_label)
            return
    ...
    def update_data(self, new_data: pd.DataFrame):
        ...
        if hasattr(self, "placeholder_label") and self.placeholder_label:
            self.widget_layout.removeWidget(self.placeholder_label)
...
def main():
    ...
    win = QMainWindow()
    win.setWindowTitle("qt")
    win.resize(800, 600)  # 設定主視窗大小
    widget = CandlestickWidget(win, title="2330(台積電)", data=create_test_data(500))
